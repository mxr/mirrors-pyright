...
This wrapper works around both those issues. Further a daily cron releases new versions in lockstep with `pyright`
releases. `node` is pinned to an LTS (24.20.0)
...
`pyright` will run in `pre-commit's` virtual env so it won't find your project dependencies by default. To work around
that, override `additional_dependencies` with your dependencies. You can also automatically keep the list up to date by
